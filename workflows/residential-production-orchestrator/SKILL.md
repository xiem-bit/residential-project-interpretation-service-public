---
name: residential-production-orchestrator
description: Use when starting, reviewing, reproducing, or changing a residential-project competition study that must begin from authorized raw inputs and produce Product 1, optional Product 2, a unified semantic core, three to four established super-competitiveness judgments, and UE solution handoffs. Do not use it merely to beautify or export an already-approved PPT or webpage.
---
...
Produce the residential business judgment chain from raw inputs. Treat PPTX and web prototypes as downstream adapters. Never use visual quality as evidence that the competition strategy was discovered.
...
8. Decide Product 1, 2, 3 and 5 enablement and high-cost admission. Product 4 is outside this release. Fewer than three established SCs means return to research; do not continue to project-specific formal carriers.
9. If Product 3 is enabled, derive Chapter 2 from customer choice to value anchor, then map every SC in Chapter 3 to customer gain, project facts, competitive relation, purchase task, UE scenes and system modules. Produce the UE handoff. For a complete new deck, first read `examples/gold-product3-public-safe/gold-authority.json`, the current 20-slide accepted gold combination and its page responsibilities, then use `references/product3/page-semantics.json` for project-specific recomposition. The current gold has three same-base real-map relationships, asymmetric SC proof depth, one lightweight AI recommender preview and a closing slide; family routes and UE blueprint are optional semantics, not its default combination. Inherit responsibilities, visual structures and expression rhythm, never source-project facts unless they are the current project's authorized inputs. Slide count, order and proof depth remain project-specific; the first review version must already be fully written, illustrated, editable, rendered, assertive-writing reviewed and visually checked.
10. If Product 5 is enabled, project the same stable IDs into its interaction blueprint. Do not create new project value downstream. Do not create files for disabled products.
...
Return to research when project identity is unresolved, a critical fact conflict changes the answer, fewer than three SCs are established, an SC lacks any of the five checks, or UE cannot prove the claimed value.

Pause only the affected branch for multiple equally valid strategic directions, reversal of an explicit user decision, material identity/rights/price/delivery conflict, or an unauthorized external action.

## Prohibited shortcuts

- Starting from `semantic-core.json` in a new research task.
- Copying tutorial `expected/`, another project’s facts, competitors, customers or SCs.
