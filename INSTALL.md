...
- 新回包绑定执行前的原请求、完整验收条件及实际充分性输入；模式、数量或质量条件冲突时拒绝，不降低原请求标准。
- 空白初始化的项目身份、输入冻结、产物和业务检查保持待完成。正式交付仍须通过原有验收。
- 区分已授权范围内迭代、明确禁止继续与范围外扩展；住宅Owner在已有授权内决定用途与续作，后续决定不改写历史请求。
- 双包离线联动从预冻结住宅请求开始，检验转换、封装和采用；通过不代表真实渠道、同事电脑或业务验收。

配套上游版本：公开信息工具包`v0.8.0-rc.6`。同时更新实际使用的上游工程目录与已安装插件，核对版本，避免新目录与旧插件缓存混用；遵循上游[固定版本安装提示](https://github.com/xiem-public-info/public-info-research-kit/blob/v0.8.0-rc.6/INSTALL_PROMPT_0.8.0-rc.6.md)。
