...
当前 `v0.2.0-rc.1` 分支已完成P0能力同构冻结，并已补入现行生产约束、增量检索、业务Harness、两套不同机制的完整公开安全项目、真人修订反馈以及产物1、2、3、5的黄金参考；自助安装与干净克隆验收仍在推进。产物4因即将进行上游大修，不进入本次公开发行范围，也没有可安装入口。候选尚未创建标签，不构成可发布版本。已发布的固定标签 `v0.1.0-rc.1` 保持不可变，并重新界定为“已冻结语义核的下游合同与平台适配演示”。请先读取 [发行状态](RELEASE_STATUS.md) 和 [能力同构合同](CAPABILITY_PARITY_CONTRACT.md)。
