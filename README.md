...
本版 `v0.2.0-rc.8` 修复上下游请求条件漂移、续作授权表达及空白任务预填完成状态；与公开信息工具包 `v0.8.0-rc.6` 联动。相对rc.7的变化及可直接复制的安装提示词见[安装与升级说明](INSTALL.md#六可直接交给codex的安装提示词)。产物1—3新版要求已接入实际规则、模板、图库和默认生成入口。产物1、2可独立交付；产物3按页面职责形成甲方方案，保留已认可的全屏地图、两侧卡片和大图版位。当前公开图库为201项，人工退出项不再进入生产。产物4只作内部衔接，产物5保持当前能力；Grist不属于依赖。旧20页PPT及三份授权原稿保留历史参考身份，当前默认实现见[代表页](examples/default-production-pages/README.md)。发行状态和边界见[发行说明](RELEASE_STATUS.md)。
...
- 按任务适用的研究与UE因果关系约束证据跨步，不把禁词扫描或机器得分当成商业判断；
- 把业务生产路径验收与 PPT／网页适配器验收彻底分开。

## 快速验证

业务合同核心使用Python标准库；含图库的完整公开检查需Pillow：
